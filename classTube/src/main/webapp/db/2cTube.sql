...
CREATE TABLE NOTE_COMMENT (
    COMMENT_NO NUMBER(5) NOT NULL,         -- 댓글 고유 ID
    MEMBER_NO NUMBER(5) NOT NULL,          -- 댓글 작성자 (FK)
    COMMENT_CONTENT VARCHAR2(1000) NOT NULL, -- 댓글 내용
    CREATE_AT DATE DEFAULT SYSDATE NOT NULL, -- 작성 날짜
    MODIFY_AT DATE,                          -- 수정 날짜 (NULL 가능)
    NOTE_NO NUMBER(5) NOT NULL,              -- 해당 게시글 ID (FK)
    CONSTRAINT PK_COMMENT PRIMARY KEY (COMMENT_NO),
    CONSTRAINT FK_NOTE_TO_COMMENT FOREIGN KEY (NOTE_NO) REFERENCES NOTE (NOTE_NO) ON DELETE CASCADE,
    CONSTRAINT FK_MEMBER_TO_COMMENT FOREIGN KEY (MEMBER_NO) REFERENCES MEMBER (MEMBER_NO) ON DELETE CASCADE
);
