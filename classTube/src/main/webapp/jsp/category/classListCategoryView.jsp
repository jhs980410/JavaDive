...
<ul class="pagination">
    <c:if test="${totalPage > 1}">
        ...
        <c:if test="${currentPage > 1}">
            <li class="page-item">
                <a class="page-link" href="/classTube/category/list?page=${currentPage - 1}${not empty keyword ? '&keyword=' : ''}${keyword}">이전</a>
            </li>
        </c:if>
        ...
        <c:forEach var="i" begin="${startPage}" end="${endPage}">
            <li class="page-item ${currentPage == i ? 'active' : ''}">
                <a class="page-link" href="/classTube/category/list?page=${i}${not empty keyword ? '&keyword=' : ''}${keyword}">${i}</a>
            </li>
        </c:forEach>
        ...
        <c:if test="${currentPage < totalPage}">
            <li class="page-item">
                <a class="page-link" href="/classTube/category/list?page=${currentPage + 1}${not empty keyword ? '&keyword=' : ''}${keyword}">다음</a>
            </li>
        </c:if>
    </c:if>
</ul>
